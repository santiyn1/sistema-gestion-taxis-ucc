...
class Nodo:
    def __init__(self, dato):
        self.dato = dato
        self.siguiente = None  # Puntero de referencia al próximo nodo enlazado


# ── TDA COLA (FIFO) — solicitudes en espera ──────────────────
class Cola:
    def __init__(self):
        self.frente = None   # Apuntador al primer elemento en salir
        self.fin = None      # Apuntador al último elemento ingresado
        self.tamanio = 0
    ...
    def encolar(self, dato):
        ...
        if self.esta_vacia():
            ...
            self.fin.siguiente = nuevo   # El último nodo de la fila se conecta con el nuevo
            self.fin = nuevo             # El puntero 'fin' se actualiza a la última posición
        self.tamanio += 1

    def encolar_al_frente(self, dato):   
        """ Reingresa una solicitud al inicio si no se encuentra conductor disponible """
        nuevo = Nodo(dato)
        nuevo.siguiente = self.frente    # El nuevo nodo apunta hacia la cabeza actual de la cola
        self.frente = nuevo              # La cabeza se reasigna al nuevo elemento
        if self.fin is None: 
            self.fin = nuevo
        self.tamanio += 1
    ...
    def desencolar(self):
        ...
        self.frente = self.frente.siguiente  # Desplaza la cabeza hacia el nodo sucesor
    ...
    def mostrar(self):
        ...
        while actual:
            s = actual.dato
            print(f"  [{pos}] #{s.id_solicitud} {s.zona_origen}->{s.zona_destino} | {s.tipo} | Estado: {s.estado}")
...
class Pila:
    def __init__(self, capacidad=10):
        self.tope = None       # Nodo ubicado en la cima actual de la pila
        self.tamanio = 0
        self.capacidad = capacidad  # Límite máximo para evitar saturación de memoria dinámica
    ...
    def apilar(self, dato):
        nuevo = Nodo(dato)
        nuevo.siguiente = self.tope   # El nuevo elemento apunta hacia abajo (tope antiguo)
        self.tope = nuevo             # El puntero principal sube al nuevo tope
        self.tamanio += 1
        if self.tamanio > self.capacidad:
            self._recortar()

    def _recortar(self):
        """ Elimina de forma automática el nodo más antiguo en el fondo de la pila """
        actual = self.tope
    ...
    def desapilar(self):
        if self.esta_vacia(): 
            return None
        dato = self.tope.dato
        self.tope = self.tope.siguiente   # El tope desciende al elemento inmediatamente inferior
        self.tamanio -= 1
        return dato
...
class ListaEnlazada:
    def __init__(self):
        self.cabeza = None   # Puntero de entrada a la lista enlazada
        self.tamanio = 0
    ...
    def agregar_al_final(self, dato):
        ...
        if self.esta_vacia():
            ...
            actual = self.cabeza
            while actual.siguiente:        # Recorrido lineal buscando el último elemento de la estructura
                actual = actual.siguiente
            actual.siguiente = nuevo       # Se crea el nuevo enlace en memoria al final de la secuencia
        self.tamanio += 1
...
class NodoBST:
    def __init__(self, conductor):
        self.dato = conductor
        self.izquierda = None   # Referencia a nodos con claves numéricas menores
        self.derecha = None     
...
class ArbolBST:
    ...
    def _insertar(self, nodo, conductor):
        if nodo is None:
            return NodoBST(conductor)
        # Ordenamiento jerárquico binario por número de identificación
        if conductor.cedula < nodo.dato.cedula:
            nodo.izquierda = self._insertar(nodo.izquierda, conductor)
        elif conductor.cedula > nodo.dato.cedula:
            nodo.derecha = self._insertar(nodo.derecha, conductor)
        return nodo
    ...
    def _buscar(self, nodo, cedula):
        ...
        if cedula == nodo.dato.cedula: 
            return nodo.dato
        # Descarte de ramas mediante búsqueda binaria de complejidad O(log n)
        if cedula < nodo.dato.cedula:
    ...
    def _inorden(self, nodo, resultados):
        """ Recorrido Izquierda -> Raíz -> Derecha para recolectar datos ordenados """
        if nodo is None: 
            return
...
class NodoVia:   # Representa las aristas del grafo
    def __init__(self, zona_destino, distancia):
        self.zona_destino = zona_destino
        self.distancia = distancia
        self.activa = True     # Control operativo de vías: False simula un cierre vial
        self.siguiente = None


class NodoZona:  # Representa los vértices del grafo
    def __init__(self, nombre):
        self.nombre = nombre
        self.lista_vias = None   # Nodo de entrada de adyacencias para este vértice específico
        self.siguiente = None
...
class Grafo:
    ...
    def agregar_via(self, zona_a, zona_b, distancia):
        """ Conexión de aristas en forma bidireccional (Grafo no dirigido) """
    ...
    def cambiar_via(self, zona_a, zona_b, abrir):
        for nombre_zona, nombre_destino in [(zona_a, zona_b), (zona_b, zona_a)]:
            ...
            while via:
                if via.zona_destino == nombre_destino: 
                    via.activa = abrir
                    break
                via = via.siguiente
    ...
    def dijkstra(self, origen):
        """ Mapeo matricial académico para hallar caminos mínimos """
        INF = float('inf')
        ...
        for _ in range(len(zonas)):
            ...
            while via:
                # Filtrado dinámico: Dijkstra evalúa la arista sólo si está activa
...
class Solicitud:
    ...
    def __init__(self, zona_origen, zona_destino, tipo):
        self.id_solicitud = Solicitud._contador
        Solicitud._contador += 1
        self.zona_origen = zona_origen
        self.zona_destino = zona_destino
        self.tipo = tipo
        self.estado = "En espera"  # Estado inicial requerido por la especificación de negocio

    def __str__(self):
        return f"#{self.id_solicitud} {self.zona_origen}->{self.zona_destino} [{self.tipo}] | Estado: {self.estado}"
...
def calcular_tarifa(distancia_metros):
    # Parámetros obligatorios de cálculo: Base $5.000 + recargo por metros
...
class SistemaTaxi:
    ...
    def atender_solicitud(self):
        ...
        solicitud.estado = "En atención"  # Cambio de estado dinámico durante el procesamiento
        ...
        if conductor.zona != solicitud.zona_origen:
            ...
            if distancia_al_origen is None:
                print("  [!] Conductor sin ruta al origen debido a bloqueos."); solicitud.estado = "En espera"
                self.cola_solicitudes.encolar_al_frente(solicitud); return
        ...
        if distancia_viaje is None:
            print("  [!] No hay ruta habilitada para el destino debido a bloqueos."); solicitud.estado = "En espera"
            self.cola_solicitudes.encolar_al_frente(solicitud); return
        ...
        solicitud.estado = "Finalizada"  # Cierre exitoso del ticket de servicio
    ...
    def cancelar_solicitud(self):
        print("\n== CANCELAR SOLICITUD ==")
        if self.cola_solicitudes.esta_vacia():
            print("  [!] No hay solicitudes para cancelar."); return
        solicitud = self.cola_solicitudes.desencolar()
        solicitud.estado = "Cancelada"  # Cierre explícito por cancelación
        
        registro = (solicitud, "Sin conductor", 0, 0, "Sin ruta", 0)
        self.historial.agregar_al_final(registro)
        self.pila_auditoria.apilar(f"CANCELÓ solicitud {solicitud.id_solicitud}")
        print(f"  ✓ Solicitud #{solicitud.id_solicitud} cancelada.")
        ...
        if idx == 0:
            ...
            if cedula_str.isdigit():
                ...
                if resultado: 
                    ...
                    print("  [!] Cédula no encontrada en el árbol.")
            else:
...
def main():
    sistema = SistemaTaxi()
    opciones = ["Registrar Solicitud","Atender Solicitud",
            "Cancelar Solicitud","Gestionar Vía",
    ...
    acciones = [sistema.registrar_solicitud, sistema.atender_solicitud, 
                sistema.cancelar_solicitud, sistema.gestionar_via, sistema.ver_historial,
                sistema.ver_cola, sistema.ver_conductores, sistema.ver_mapa]
    while True:
        # Invocación automática de limpieza de consola mediante el TDA OS nativo del sistema operativo
        ...
        entrada = input("  Opción (1-9): ").strip()
        if entrada == "9": 
            print("\n  ¡Hasta luego!\n"); break
        if entrada.isdigit() and 1 <= int(entrada) <= 8:
